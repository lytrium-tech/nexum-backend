...
class ConversationalResponse(BaseModel):
    ...
    response_text: str
    intent: str
    status: Literal["completed", "awaiting_clarification", "awaiting_confirmation", "cancelled", "error"]
    trace_id: uuid.UUID
    pending_action_id: uuid.UUID | None = None
...
class ExtractedEntities(BaseModel):
    ...
    obligation: str | None = Field(None, description="Nombre de la obligación mencionada. Ej. Arriendo")
    installments_total: int | None = Field(None, description="Número de cuotas de una compra con TC. Ej. 1, 12, 36")
    description: str | None = Field(None, description="Descripción adicional del gasto o ingreso.")
    target_date: str | None = Field(None, description="Fecha objetivo en formato YYYY-MM-DD para metas.")
