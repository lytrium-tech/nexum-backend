...
class GeminiClient:
    ...
    def extract_intent_and_entities(
        self, message: str, system_prompt: str, retries: int = 1
        ...
        while attempts <= retries:
            ...
            try:
                response = self.client.models.generate_content(
                    model=current_model, contents=[message], config=config
                )
                ...
                if attempts > retries:
                    ...
                    raise GeminiClientError(
                        f"Fallo al comunicarse con Gemini después de varios intentos: {str(e)}"
                    )
