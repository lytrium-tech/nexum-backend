...
def calculate_period_bounds(obligation: Obligation, sequence_number: int) -> tuple[date, date, date]:
    ...
    if freq == "monthly":
        target_month_date = add_months(start.replace(day=1), (sequence_number - 1) * obligation.interval_count)
        ...
        target_year = start.year + ((sequence_number - 1) * obligation.interval_count)
        ...
        days_per_interval = 7 * obligation.interval_count
        ...
        days_per_interval = 14 * obligation.interval_count
        days_shift = (sequence_number - 1) * days_per_interval
...
class PeriodEngine:
    ...
    def _find_sequence_for_date(self, obligation: Obligation, d: date) -> int:
        ...
        if obligation.frequency == "monthly":
            months_diff = (d.year - obligation.start_date.year) * 12 + (d.month - obligation.start_date.month)
            return (months_diff // obligation.interval_count) + 1
        elif obligation.frequency == "yearly":
            years_diff = d.year - obligation.start_date.year
            return (years_diff // obligation.interval_count) + 1
        elif obligation.frequency == "weekly":
            days_diff = (d - obligation.start_date).days
            return (days_diff // (7 * obligation.interval_count)) + 1
        elif obligation.frequency == "biweekly":
            days_diff = (d - obligation.start_date).days
            return (days_diff // (14 * obligation.interval_count)) + 1
        return 1

    async def _ensure_period_exists(self, obligation: Obligation, sequence_number: int, current_date: date) -> ObligationPeriod:
