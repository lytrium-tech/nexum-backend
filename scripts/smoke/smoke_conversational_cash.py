...
async def run_smoke():
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        ...
        if acc_id:
            await client.patch(
                f"{API_URL}/accounts/{acc_id}/balance",
        ...
        if pending_action_id:
            ...
            resp_conf2 = await client.post(
                f"{API_URL}/conversations/message", json=confirm_payload2, headers=AUTH_HEADERS
            )
        ...
        resp = await client.post(
            f"{API_URL}/conversations/message", json=msg_payload_exp, headers=AUTH_HEADERS
        )
        data = resp.json()
