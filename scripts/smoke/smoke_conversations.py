...
# Importante: Mockear Gemini para que el script no dependa de la API key real
# Parcheamos la clase en el módulo integrado
...
async def send_message(client, msg, external_id=None, pending_action_id=None):
    ...
    resp = await client.post(
        f"{BASE_URL}/api/v1/conversations/message", json=payload, headers=HEADERS
    )
    ...
    async with httpx.AsyncClient(timeout=30) as client:
        ...
        if not any(a["name"] == "Nequi" for a in accounts):
            resp = await client.post(
                ...
                json={"name": f"Nequi {uuid.uuid4().hex[:6]}", "type": "wallet"},
                headers=HEADERS,
            )
            resp.raise_for_status()
        ...
        if not any(c["name"] == "Comida" for c in categories):
            resp = await client.post(
                ...
                json={"name": f"Comida {uuid.uuid4().hex[:6]}", "type": "expense"},
                headers=HEADERS,
            )
            resp.raise_for_status()
        ...
        await client.post(
            ...
            json={"amount": 1000, "account_id": nequi_id},
            headers={**HEADERS, "Idempotency-Key": str(uuid.uuid4())},
        )

        # 2. Mockeamos la API de Gemini
        from app.conversations.schemas import ExtractedEntities, GeminiNLUOutput
        from app.integrations.gemini_client import GeminiClient

        def mock_extract(self, message, system_prompt, retries=1):
            msg = message.lower()
            if "cuánto dinero tengo" in msg:
                nlu = GeminiNLUOutput(
                    intent="ask_balance", entities=ExtractedEntities(), confidence=1.0
                )
            elif "cuánto dinero libre" in msg:
                nlu = GeminiNLUOutput(
                    intent="ask_free_money", entities=ExtractedEntities(), confidence=1.0
                )
            elif "gasté 20 en comida desde nequi" in msg:
                nlu = GeminiNLUOutput(
                    intent="create_expense",
                    entities=ExtractedEntities(amount=20, category="comida", account="nequi"),
                    confidence=1.0,
                )
            elif "tarjeta ambigua" in msg:
                # Simulamos ambigüedad extrayendo "tarjeta" si hay multiples
                nlu = GeminiNLUOutput(
                    intent="create_credit_card_purchase",
                    entities=ExtractedEntities(amount=100, credit_card="tarjeta"),
                    confidence=1.0,
                )
            else:
                nlu = GeminiNLUOutput(
                    intent="unknown", entities=ExtractedEntities(), confidence=1.0
                )
        ...
        with patch.object(GeminiClient, "extract_intent_and_entities", new=mock_extract):
            # 3. Preguntar balance
            ...
            # 7. Verificar duplicados de confirmación
            print("Reenviando confirmación: Sí")
            resp6 = await send_message(client, "Sí", pending_action_id=pending_action_id)
            assert resp6["response_text"] == "Esta acción ya fue ejecutada previamente."
