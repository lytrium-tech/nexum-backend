...
async def main():
    ...
    async with async_session() as session:
        ...
        acc = await account_service.create_account(
            user.id,
            AccountCreate(
                name="Bancolombia", type="bank", currency="COP", balance=Decimal("1000000.00")
            ),
        )
        ...
        res1 = await credit_service.create_purchase(
            ...
            CreditCardPurchaseCreate(
                amount=Decimal("150000"), installments_total=1, description="Zapatos"
            ),
            cmd1,
        )
        await session.commit()
        ...
        res2 = await credit_service.create_purchase(
            ...
            card.id,
            CreditCardPurchaseCreate(
                amount=Decimal("50000"), installments_total=1, description="Cena"
            ),
            cmd2,
        )
        await session.commit()
        ...
        res_cuotas = await credit_service.create_purchase(
            ...
            card.id,
            CreditCardPurchaseCreate(
                amount=Decimal("300000"), installments_total=6, description="Electrodomestico"
            ),
            cmd_cuotas,
        )
        await session.commit()
