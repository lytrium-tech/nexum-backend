...
async def run_smoke():
    ...
    async with httpx.AsyncClient(timeout=10.0) as client:
        ...
        r = await client.post(
            ...
            json={"name": f"Fixed {uuid.uuid4().hex[:6]}", "amount": "500", "payment_mode": "fixed_full_payment"},
            headers=headers,
        )
        r.raise_for_status()
        fixed_id = r.json()["id"]
        ...
        r = await client.post(
            f"{base_url}/api/v1/obligations",
            json={"name": f"Partial {uuid.uuid4().hex[:6]}", "amount": "400", "payment_mode": "partial_allowed"},
            headers=headers,
        )
        r.raise_for_status()
        ...
        r = await client.post(
            f"{base_url}/api/v1/obligations",
            json={"name": f"Unpaid {uuid.uuid4().hex[:6]}", "amount": "300", "payment_mode": "fixed_full_payment"},
            headers=headers,
        )
        r.raise_for_status()
        ...
        diff = float(snapshot2["truth"]["committed_outflows"]) - float(snapshot["truth"]["committed_outflows"])
