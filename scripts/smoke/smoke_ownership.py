...
async def create_test_user(
    client: httpx.AsyncClient, name: str, email: str, internal: bool = False
):
...
async def main():
    async with httpx.AsyncClient() as client:
        ...
        headers_a = {
            "Authorization": f"Bearer {auth_a}",
            "X-Test-Bypass-Auth": "true",
            "Content-Type": "application/json",
        }
        headers_b = {
            "Authorization": f"Bearer {auth_b}",
            "X-Test-Bypass-Auth": "true",
            "Content-Type": "application/json",
        }
        ...
        resp = await client.post(
            f"{BASE_URL}/accounts",
            json={"name": f"Alice Wallet {run_id}", "type": "cash", "currency": "COP"},
            headers=headers_a,
        )
        resp.raise_for_status()
        ...
        resp = await client.patch(
            f"{BASE_URL}/accounts/{acc_a_id}", json={"name": f"Hacked {run_id}"}, headers=headers_b
        )
        assert resp.status_code in (403, 404), f"Expected 403/404, got {resp.status_code}"
        print("  -> Access denied (OK)")
        ...
        resp = await client.post(
            ...
            json={"name": f"Alice Secrets {run_id}", "type": "expense"},
            headers=headers_a,
        )
        resp.raise_for_status()
        ...
        resp = await client.patch(
            f"{BASE_URL}/categories/{cat_a_id}",
            json={"name": f"Hacked {run_id}"},
            headers=headers_b,
        )
        assert resp.status_code in (403, 404), f"Expected 403/404, got {resp.status_code}"
        print("  -> Access denied (OK)")
        ...
        resp = await client.post(
            f"{BASE_URL}/goals",
            json={"name": f"Alice Car {run_id}", "target_amount": 10000.0},
            headers=headers_a,
        )
        resp.raise_for_status()
        ...
        resp = await client.post(
            f"{BASE_URL}/accounts",
            json={"name": f"Bob Wallet {run_id}", "type": "cash", "currency": "COP"},
            headers=headers_b,
        )
        acc_b_id = resp.json()["id"]

        resp = await client.post(
            f"{BASE_URL}/goals/{goal_a_id}/contributions",
            json={"amount": 100.0, "account_id": acc_b_id},
            headers=headers_b,
        )
        assert resp.status_code in (403, 404), f"Expected 403/404, got {resp.status_code}"
        print("  -> Access denied (OK)")
        ...
        resp = await client.post(
            ...
            json={"name": f"Alice Rent {run_id}", "amount": 500.0, "frequency": "monthly"},
            headers=headers_a,
        )
        resp.raise_for_status()
        ...
        resp = await client.post(
            f"{BASE_URL}/obligations/{obl_a_id}/payments",
            json={"amount": 500.0, "account_id": acc_b_id},
            headers=headers_b,
        )
        assert resp.status_code in (403, 404), f"Expected 403/404, got {resp.status_code}"
        print("  -> Access denied (OK)")
        ...
        resp = await client.post(
            f"{BASE_URL}/credit/cards/{cc_a_id}/purchases",
            json={
                "amount": 100.0,
        ...
        resp = await client.post(
            f"{BASE_URL}/credit/cards/{cc_a_id}/payments",
            json={"amount": 100.0, "account_id": acc_b_id},
