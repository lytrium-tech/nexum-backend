...
async def main():
    ...
    async with AsyncClient(base_url="http://localhost:8000", timeout=30.0) as client:
        ...
        resp = await client.post(
            "/api/v1/accounts",
            json={"name": "Nequi", "type": "cash", "currency": "COP"},
            headers=headers,
        )
        ...
        resp = await client.post(
            "/api/v1/accounts",
            json={"name": "Bancolombia", "type": "cash", "currency": "COP"},
            headers=headers,
        )
        if resp.status_code == 409:
            pass  # already exists
        ...
        resp = await client.post(
            ...
            json={"message": "me entraron 500 mil a Nequi de sueldo", "channel": "api"},
            headers=headers,
        )
        print("Ingreso Nequi:")
        print(resp.json())
        ...
        resp = await client.post(
            "/api/v1/conversations/message",
            json={"message": "transferí 200 mil de Nequi a Bancolombia", "channel": "api"},
            headers=headers,
        )
        print("\nTransfers Intent:")
        print(resp.json())
        ...
        for ev in resp.json()["items"][:4]:
            print(
                f"- {ev['event_type']} | {ev['direction']} | {ev['amount']} | Account: {ev['account']['name']}"
            )
