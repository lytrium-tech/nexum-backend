...
async def create_test_user(client, name: str, email: str) -> str:
    ...
    resp = await client.post(f"{API_URL}/users/me/bootstrap", json={
        "name": name,
...
async def main():
    async with httpx.AsyncClient(base_url=API_URL, timeout=10.0) as client:
        ...
        # 1. Crear cuenta
        r = await client.post("/accounts", headers=h_a, json={
            "name": "Cuenta A",
            "type": "wallet",
            "currency": "COP"
        })
        ...
        # 2. Editar cuenta
        r = await client.patch(f"/accounts/{acc_a['id']}", headers=h_a, json={
            "name": "Cuenta A Editada"
        })
        r.raise_for_status()
        ...
        r = await client.post("/categories", headers=h_a, json={"name": "Privada A", "type": "expense"})
        r.raise_for_status()
        cat_a = r.json()
        ...
        r = await client.patch(f"/categories/{global_cat['id']}", headers=h_b, json={"name": "Hacked global"})
        ...
        msg_payload_inc = {"channel": "api", "external_message_id": "998", "message": "Me pagaron 50 mil en Cuenta A Editada"}
        r = await client.post("/conversations/message", headers=h_a, json=msg_payload_inc)
        r.raise_for_status()
        data_inc = r.json()
        assert data_inc["intent"] == "create_income"
        
        r = await client.post("/conversations/message", headers=h_a, json={
            "channel": "api", "external_message_id": "998b", "message": "si"
        })
        r.raise_for_status()
        ...
        msg_payload = {"channel": "api", "external_message_id": "999", "message": "Gaste 10 mil en Privada A desde Cuenta A Editada"}
        ...
        # Confirmar
        r = await client.post("/conversations/message", headers=h_a, json={
            "channel": "api", "external_message_id": "1000", "message": "si"
        })
        r.raise_for_status()
        data2 = r.json()
        ...
        msg_payload = {"channel": "api", "external_message_id": "1001", "message": "Gaste 5 mil en algo_inventado desde Cuenta A Editada"}
        r = await client.post("/conversations/message", headers=h_a, json=msg_payload)
        r.raise_for_status()
        data3 = r.json()
        ...
        assert "clasificar" in data3.get("response_text", "").lower() or data3.get("intent") == "create_expense", "Fallback category not used"
