...
async def conversational_flow(client: httpx.AsyncClient, message: str, label: str):
    ...
    resp = await client.post(f"{API_URL}/conversations/message", json=msg_payload, headers=AUTH_HEADERS)
    data = resp.json()
    ...
    if action_id:
        ...
        resp_conf = await client.post(f"{API_URL}/conversations/message", json=conf_payload, headers=AUTH_HEADERS)
        ...
        resp_conf2 = await client.post(f"{API_URL}/conversations/message", json=conf2_payload, headers=AUTH_HEADERS)
...
async def run_smoke():
    async with httpx.AsyncClient() as client:
        ...
        fund_resp = await client.post(f"{API_URL}/conversations/message", json=msg_payload_inc, headers=AUTH_HEADERS)
        act_id = fund_resp.json().get("pending_action_id")
        if act_id:
            await client.post(f"{API_URL}/conversations/message", json={"channel": "api", "message": "sí", "pending_action_id": act_id, "external_message_id": str(uuid.uuid4())}, headers=AUTH_HEADERS)

        # GOALS
        goal_name = f"Viaje_{uuid.uuid4().hex[:6]}"
        await conversational_flow(client, f"Quiero crear una meta llamada {goal_name} por 2000000 para 2026-12-31", "Create Goal")
        ...
        # Contribute to Goal
        await conversational_flow(client, f"Aporta 100000 a la meta {goal_name} desde la cuenta {acc_name}", "Goal Contribution")
        
        # Verify Goal
        goal_verify = await client.get(f"{API_URL}/goals/{goal['id']}", headers=AUTH_HEADERS)
        assert float(goal_verify.json()["current_amount"]) == 100000.0, "Goal current amount incorrect"

        # OBLIGATIONS
        obl_name = f"Arriendo_{uuid.uuid4().hex[:6]}"
        await conversational_flow(client, f"Crea una obligación mensual de {obl_name} por 900000", "Create Obligation")
        ...
        await conversational_flow(client, f"Pagué 900000 del {obl_name} desde la cuenta {acc_name}", "Pay Obligation")

        # CREDIT
        card_name = f"Nu_{uuid.uuid4().hex[:6]}"
        card_resp = await client.post(f"{API_URL}/credit/cards", json={
            "name": card_name, "bank": "Nu", "credit_limit": 5000000, "cutoff_day": 15, "due_day": 30
        }, headers=AUTH_HEADERS)
        card_id = card_resp.json()["id"]

        # Purchase to create debt
        await client.post(f"{API_URL}/credit/cards/{card_id}/purchases", json={
            "amount": 300000, "installments_total": 1, "description": "Compra de prueba"
        }, headers={**AUTH_HEADERS, "Idempotency-Key": str(uuid.uuid4())})

        await conversational_flow(client, f"Pagué 300000 de la tarjeta {card_name} desde {acc_name}", "Pay Credit Card")
        ...
        assert float(data["estimated_current_debt"]) == 0.0, f"Credit card debt not correctly paid. Got: {data}"
