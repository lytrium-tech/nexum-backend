    ...
    try:
        async with engine.connect() as conn:
            goal_res = await conn.execute(
                text(
                    "SELECT id, name, target_amount, current_amount, progress_percentage, status FROM public.goals WHERE id = :gid"
                ),
                {"gid": goal_id},
            )
            ...
            if goal_row:
                logger.info(
                    f" -> Meta BD: {goal_row.name}, Target: {goal_row.target_amount}, Current: {goal_row.current_amount}, Progress: {goal_row.progress_percentage}%, Status: {goal_row.status}"
                )
...
async def run_smoke():
    ...
    async with httpx.AsyncClient(timeout=10.0) as client:
        ...
        r = await client.post(
            ...
            json={"name": "Meta Smoke Goals", "target_amount": "300"},
            headers=headers,
        )
        r.raise_for_status()
        goal_id = r.json()["id"]
        ...
        assert float(data["goal_current_amount"]) == 100.0
        ...
        # 13 y 14. Aporte 200 (Completa meta)
        cmd2 = str(uuid.uuid4())
        r = await client.post(
            f"{base_url}/api/v1/goals/{goal_id}/contributions",
