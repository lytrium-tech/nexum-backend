...
async def get_test_user(conn):
    ...
    if not goal_id:
        goal_id = uuid.uuid4()
        await conn.execute(
            "INSERT INTO goals (id, user_id, name, target_amount, status) VALUES ($1, $2, $3, $4, $5)",
            goal_id,
            user_id,
            "Viaje",
            10000000,
...
async def simulate_message(
    client: httpx.AsyncClient, token: str, message: str, pending_action_id: str = None
) -> dict:
    ...
    resp = await client.post(
        f"{API_URL}/api/v1/conversations/message", json=payload, headers=headers
    )
...
async def get_auth_token(client: httpx.AsyncClient) -> str:
    resp = await client.post(
        f"{API_URL}/api/v1/auth/token", data={"username": "dev@nexum.local", "password": "password"}
    )
    ...
    await client.post(
        ...
        json={"email": "dev@nexum.local", "password": "password", "full_name": "Dev User"},
    )
    resp = await client.post(
        f"{API_URL}/api/v1/auth/token", data={"username": "dev@nexum.local", "password": "password"}
    )
...
async def main():
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        ...
        await conn.execute(
            "UPDATE pending_actions SET status = 'cancelled' WHERE user_id = $1", user_id
        )
        ...
        assert r["status"] == "awaiting_confirmation", (
            f"Debería estar en awaiting_confirmation, obtuvo {r['status']}"
        )
        ...
        open_count = await conn.fetchval(
            "SELECT count(*) FROM pending_actions WHERE user_id = $1 AND status IN ('awaiting_confirmation', 'awaiting_clarification') AND expires_at > now()",
            user_id,
        )
