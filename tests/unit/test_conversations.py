...
async def test_handle_message_read_intent(
    mock_gemini, conversations_service, mock_intel_service, mock_repo
):
    user_id = uuid.uuid4()
    AuthenticatedIdentity(user_id=str(user_id))
    trace_id = uuid.uuid4()
    ...
    nlu_out = GeminiNLUOutput(intent="ask_balance", entities=ExtractedEntities(), confidence=0.9)
    metadata = {
...
async def test_pending_action_confirm_natural(conversations_service, mock_repo, mock_cash_service):
    ...
    req = ConversationalRequest(
        message="sí lo confirmo", channel="api", pending_action_id=str(action_id)
    )
...
async def test_pending_action_confirm_unsupported_intent(conversations_service, mock_repo):
    ...
    mock_repo.update_pending_action_status.assert_called_once_with(
        action_id,
        "awaiting_confirmation",
        confirmation_message_id=mock_repo.save_message.return_value,
    )


@pytest.mark.asyncio
async def test_pending_action_confirm_create_goal(
    conversations_service, mock_repo, mock_goals_service
):
    user_id = uuid.uuid4()
    trace_id = uuid.uuid4()
    action_id = uuid.uuid4()
...
@pytest.mark.asyncio
async def test_pending_action_confirm_create_goal_contribution(
    conversations_service, mock_repo, mock_goals_service
):
    user_id = uuid.uuid4()
    trace_id = uuid.uuid4()
    ...
    action = PendingActionRead(
        id=action_id,
        user_id=user_id,
        intent="create_goal_contribution",
...
async def test_pending_action_confirm_create_obligation_payment(
    ...
    action = PendingActionRead(
        id=action_id,
        user_id=user_id,
        intent="create_obligation_payment",
        data={
            "amount": 900000,
...
@pytest.mark.asyncio
async def test_pending_action_confirm_create_credit_card_payment(
    conversations_service, mock_repo, mock_credit_service
):
    user_id = uuid.uuid4()
    trace_id = uuid.uuid4()
    action_id = uuid.uuid4()
...
async def test_pending_action_clarification_merge(mock_gemini, conversations_service, mock_repo):
    ...
    nlu_out = GeminiNLUOutput(
        intent="unknown", entities=ExtractedEntities(account="Nequi"), confidence=1.0
    )
    mock_gemini.extract_intent_and_entities.return_value = (
        nlu_out,
        {
...
@pytest.mark.asyncio
@patch("app.conversations.service.gemini_client")
async def test_pending_action_clarification_intent_change(
    mock_gemini, conversations_service, mock_repo
):
    user_id = uuid.uuid4()
    trace_id = uuid.uuid4()
    action_id = uuid.uuid4()
    req = ConversationalRequest(
        message="Mejor crea una meta Viaje por 1 millón",
        channel="api",
    ...
    nlu_out = GeminiNLUOutput(
        intent="create_goal",
        entities=ExtractedEntities(amount=1000000, goal="Viaje"),
        confidence=1.0,
    )
    mock_gemini.extract_intent_and_entities.return_value = (
        nlu_out,
