...
async def test_get_latest_rate_success(client: AsyncClient, token_headers: dict[str, str]):
    response = await client.get("/api/v1/fx/rates/latest?base_currency=USD&quote_currency=COP", headers=token_headers)
...
@pytest.mark.asyncio
async def test_get_latest_rate_feature_flag_off(client: AsyncClient, token_headers: dict[str, str], monkeypatch):
