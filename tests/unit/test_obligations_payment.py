...
def mock_account_repo():
    ...
    acc = Account(id=uuid.uuid4(), user_id=uuid.uuid4(), currency="COP", balance=Decimal("10000.00"))
...
def service(mock_repo, mock_account_repo, mock_ledger_service):
    ...
    s._calculate_fx_and_amounts = AsyncMock(return_value=(Decimal("100.00"), Decimal("100.00"), Decimal("1.0"), "mock", datetime.now()))
    return s
...
async def test_pay_specific_period_partial(service, base_user_id, mock_repo, mock_account_repo):
    ...
    period = ObligationPeriod(
        id=uuid.uuid4(), obligation_id=uuid.uuid4(), amount=Decimal("100.00"), paid_amount=Decimal("0.00"), status="pending_payment"
    )
    mock_res = MagicMock()
    ...
    service._calculate_fx_and_amounts.return_value = (Decimal("40.00"), Decimal("40.00"), Decimal("1.0"), None, None)
...
async def test_pay_specific_period_full(service, base_user_id, mock_repo, mock_account_repo):
    ...
    period = ObligationPeriod(
        id=uuid.uuid4(), obligation_id=uuid.uuid4(), amount=Decimal("100.00"), paid_amount=Decimal("10.00"), status="partially_paid"
    )
    mock_res = MagicMock()
    mock_res.scalar_one_or_none.return_value = period
    mock_repo.session.execute.return_value = mock_res
    ...
    service._calculate_fx_and_amounts.return_value = (Decimal("90.00"), Decimal("90.00"), Decimal("1.0"), None, None)
...
@pytest.mark.asyncio
async def test_pay_specific_period_overpayment_rejected(service, base_user_id, mock_repo, mock_account_repo):
    acc = mock_account_repo.get_by_id_for_update.return_value
    acc.user_id = base_user_id
    ...
    mock_res.scalar_one_or_none.return_value = period
    mock_repo.session.execute.return_value = mock_res
    mock_repo.get_by_id.return_value = Obligation(id=period.obligation_id, user_id=base_user_id, currency="COP")

    service._calculate_fx_and_amounts.return_value = (Decimal("150.00"), Decimal("150.00"), Decimal("1.0"), None, None)
...
@pytest.mark.asyncio
async def test_pay_specific_period_blocks_variable_undefined(service, base_user_id, mock_repo, mock_account_repo):
    acc = mock_account_repo.get_by_id_for_update.return_value
    acc.user_id = base_user_id
...
@pytest.mark.asyncio
async def test_pay_fifo_pays_overdue_before_current(service, base_user_id, mock_repo, mock_account_repo):
    acc = mock_account_repo.get_by_id_for_update.return_value
    acc.user_id = base_user_id
    ...
    p1 = ObligationPeriod(id=uuid.uuid4(), obligation_id=obligation.id, amount=Decimal("100.00"), paid_amount=Decimal("0.00"), status="overdue")
    p2 = ObligationPeriod(id=uuid.uuid4(), obligation_id=obligation.id, amount=Decimal("100.00"), paid_amount=Decimal("0.00"), status="pending_payment")
...
async def test_pay_fifo_rejects_overpayment(service, base_user_id, mock_repo, mock_account_repo):
    ...
    p1 = ObligationPeriod(id=uuid.uuid4(), obligation_id=obligation.id, amount=Decimal("100.00"), paid_amount=Decimal("0.00"), status="pending_payment")
...
@pytest.mark.asyncio
async def test_pay_full_with_null_amount_calculates_balance(service, base_user_id, mock_repo, mock_account_repo):
    ...
    p1 = ObligationPeriod(id=uuid.uuid4(), obligation_id=obligation.id, amount=Decimal("75.00"), paid_amount=Decimal("0.00"), status="pending_payment")
...
async def test_pay_cross_currency_cop_to_usd(mock_get_fx, service, base_user_id, mock_repo, mock_account_repo):
    real_service = ObligationService(mock_repo)
    real_service.account_repo = mock_account_repo
    real_service.ledger_service = service.ledger_service
    ...
    p1 = ObligationPeriod(id=uuid.uuid4(), obligation_id=obligation.id, amount=Decimal("20.00"), paid_amount=Decimal("0.00"), status="pending_payment")
    ...
    with patch("app.obligations.service.PeriodEngine") as MockEngine:
        ...
        payload = ObligationPaymentCreate(account_id=acc.id, amount=Decimal("40000.00"), currency="COP")
        await real_service.pay_obligation_fifo(base_user_id, obligation.id, payload)
...
@pytest.mark.asyncio
async def test_pay_insufficient_funds_doesnt_change_period(service, base_user_id, mock_repo, mock_account_repo):
    acc = mock_account_repo.get_by_id_for_update.return_value
    acc.user_id = base_user_id
    ...
    service._calculate_fx_and_amounts.return_value = (Decimal("50.00"), Decimal("50.00"), Decimal("1.0"), None, None)
...
async def test_pay_unpayable_states(service, base_user_id, mock_repo, mock_account_repo):
    ...
    period = ObligationPeriod(
        id=uuid.uuid4(), obligation_id=uuid.uuid4(), amount=Decimal("100.00"), paid_amount=Decimal("100.00"), status="paid"
    )
    mock_res = MagicMock()
    mock_res.scalar_one_or_none.return_value = period
    mock_repo.session.execute.return_value = mock_res
