...
@pytest.mark.asyncio
async def test_pay_specific_period_cross_currency_debits_source_amount_not_applied_amount(
    mock_db, monkeypatch
):
    import uuid
    from datetime import UTC, date, datetime, timedelta
    ...
    payload = {
        "idempotency_key": "test-req-debit-fix-1",
        "amount": 20000,
...
@pytest.mark.asyncio
async def test_pay_specific_period_cross_currency_validations(mock_db, monkeypatch):
    import uuid
    from datetime import UTC, date, datetime, timedelta
    from decimal import Decimal
    from unittest.mock import MagicMock
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        # Case 1: Cross-currency payment without quote_id fails
        payload_no_quote = {
            "idempotency_key": "test-req-debit-fix-no-quote",
            "amount": 20000,
            "source_account_id": str(account_id),
        }
        ...
        payload_mismatched_amount = {
            "idempotency_key": "test-req-debit-fix-mismatched-amount",
            "amount": 15000,
            "source_account_id": str(account_id),
            "quote_id": str(quote_id),
        }
        res = await client.post(
            f"/api/v1.7/obligations/{obs_id}/periods/{period_id}/payments",
            json=payload_mismatched_amount,
        )
        assert res.status_code == 422
        assert res.json()["detail"] == "invalid_fx_quote"

        # Case 3: Quote with mismatched source currency (meaning mismatched source_account_id) fails
        mock_account.currency = "EUR"
        payload_mismatched_currency = {
            "idempotency_key": "test-req-debit-fix-mismatched-currency",
            "amount": 20000,
            "source_account_id": str(account_id),
            "quote_id": str(quote_id),
        }
        res = await client.post(
            f"/api/v1.7/obligations/{obs_id}/periods/{period_id}/payments",
            json=payload_mismatched_currency,
        )
        assert res.status_code == 422
        assert res.json()["detail"] == "invalid_fx_quote"
