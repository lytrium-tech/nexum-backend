...
async def test_conversational_response_multiple_actions_no_cop():
    ...
    svc = ConversationsService(
        repo_mock, AsyncMock(), AsyncMock(), AsyncMock(), AsyncMock(), AsyncMock(), AsyncMock()
    )
    svc.repo = repo_mock
    ...
    resp = await svc._build_and_save_multiple_actions_response(
        uuid4(), uuid4(), req, uuid4(), actions
    )
...
async def test_conversational_create_goal_dynamic_currency():
    ...
    svc = ConversationsService(
        repo_mock, AsyncMock(), AsyncMock(), goals_mock, AsyncMock(), AsyncMock(), AsyncMock()
    )
    svc.repo = repo_mock
    ...
    action = PendingActionRead(
        ...
        updated_at=datetime.now(UTC),
        command_id=uuid4(),
        data={"name": "test", "target_amount": 100, "currency": "USD"},
    )
...
async def test_conversational_create_obligation_dynamic_currency():
    ...
    obl_mock = AsyncMock()
    svc = ConversationsService(
        repo_mock, AsyncMock(), AsyncMock(), AsyncMock(), obl_mock, AsyncMock(), AsyncMock()
    )
    svc.repo = repo_mock
    ...
    action = PendingActionRead(
        ...
        updated_at=datetime.now(UTC),
        command_id=uuid4(),
        data={"name": "test", "amount": 100, "currency": "EUR"},
    )
    await svc._execute_financial_action(uuid4(), action)
...
async def test_conversational_create_income_no_currency():
    ...
    cash_mock = AsyncMock()
    svc = ConversationsService(
        repo_mock, AsyncMock(), cash_mock, AsyncMock(), AsyncMock(), AsyncMock(), AsyncMock()
    )
    svc.repo = repo_mock
    ...
    action = PendingActionRead(
        ...
        updated_at=datetime.now(UTC),
        command_id=uuid4(),
        data={"account_id": str(uuid4()), "amount": 100},
    )
    await svc._execute_financial_action(uuid4(), action)
